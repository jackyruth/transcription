...
class MarkdownFormatter:
    """
    A class for formatting markdown files by organizing sentences and wrapping text.
    
    Processes markdown content by:
    - Splitting text into sentences
    - Wrapping lines to specified width
    - Preserving markdown headers and code blocks
    """
    ...
    def _wrap_line(self, text: str) -> str:
        ...
        for word in words:
            ...
            if current_length + len(word) + (1 if current_line else 0) <= self.line_width:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python format_markdown.py <input_file> [output_file]", file=sys.stderr)
        print("If output_file is not provided, result is printed to stdout.", file=sys.stderr)
        sys.exit(1)
    ...
    formatter.process_file(input_file, output_file)
